...
3. Run `supabase/schema.sql` in the Supabase SQL editor to create the tables and seed `Chicago, IL` into `locations`.
4. Start both services locally with `npm run dev`.
5. Or, if you want to run them separately, use `npm run dev:web` and `npm run dev:worker`.
...
- The frontend shows Supabase-backed locations plus a browser-powered `Use my live location` option.
- `Use my live location` fetches one immediate WattTime snapshot through the Next.js server, stores that location in Supabase, and then lets the Railway worker continue polling it on later refresh cycles.
